...
export async function createDonationApi(
  requestData: DonationItemDTO
): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.post(
      `${urls.baseUrl}${urls.donation}`,
      requestData
    );
...
export async function editDonationApi(
  id: number,
  requetData: DonationItemDTO
): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.put(
      `${urls.baseUrl}${urls.updateDonation}/${id}`,
      requetData
    );
    let data: DonationItemDTO;
    if (res.status) {
...
export async function makeDonationApi(
  payload: PaymentDTO
): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.post(`${urls.baseUrl}${urls.Payment}`, payload);
    if (res.status) {
      response.data = res.data;
      response.code = statusEnum.ok;
    }
