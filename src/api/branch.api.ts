...
export async function createBranchApi(requetData: BranchDTO): Promise<ResponseDTO> {
   ...
   try {
      let res = await request.post(`${urls.baseUrl}${urls.createBranch}`, requetData);
      //alert(JSON.stringify(res));
      // const hashlidEncoDecode: HashlidEncoDecode = new HashlidEncoDecode(saltConst);
      let data: BranchDTO;
      if (res.status) {
...
export async function editBranchApi(id: number, requetData: BranchDTO): Promise<ResponseDTO> {
   ...
   try {
      let res = await request.put(`${urls.baseUrl}${urls.updateBranch}/${id}`, requetData);
      let data: BranchDTO;
      if (res.status) {
         data = res.data;
...
export async function branchAssignAdminApi(requetData: BranchAssignAdminDTO): Promise<ResponseDTO> {
   ...
   try {
      let res = await request.put(`${urls.baseUrl}${urls.branchAssignAdmin}`, requetData);
      let data: BranchDTO;
      if (res.status) {
         data = res.data;
