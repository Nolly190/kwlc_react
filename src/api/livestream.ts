...
export async function createStreamApi(requestData: LiveStreamDTO, id: number): Promise<ResponseDTO> {
   ...
   try {
      let res = await request.post(`${urls.baseUrl}${urls.liveStream}/${id}`, requestData);
...
export async function editStreamApi(requestData: LiveStreamDTO, id: number): Promise<ResponseDTO> {
   ...
   try {
      let res = await request.put(`${urls.baseUrl}${urls.liveStream}/${urls.updateLiveStream}${id}`, requestData);
      let data: LiveStreamDTO;
      if (res.status) {
         data = res.data;
         response.data = data;
