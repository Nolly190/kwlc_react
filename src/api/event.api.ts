...
import { CreateEventPayload, CreateEventTypePayload, EventsResponse, EventTypeResponse } from "../types/appTypes";
...
export async function getEventsApi(): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.get(`${urls.baseUrl}${urls.GetAllEvents}`);
...
export async function createEventApi(requestData: CreateEventPayload): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.post(`${urls.baseUrl}${urls.AddEvent}`, requestData);
...
export async function updateEventApi(requestData: CreateEventPayload, id: number): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.put(`${urls.baseUrl}${urls.UpdateEvent}/${id}`, requestData);
    let data: CreateEventPayload;
    if (res.status) {
      data = res.data;
      response.data = data;
...
export async function createEventTypeApi(requestData: CreateEventTypePayload): Promise<ResponseDTO> {
  const response = new ResponseDTO();

  try {
    let res = await request.post(`${urls.baseUrl}${urls.AddEventType}`, requestData);
    if (res.status) {
...
export async function getEventsByTypeApi(filter: string): Promise<ResponseDTO> {
  ...
  try {
    let res = await request.get(`${urls.baseUrl}${urls.GetEventsByType}?filter=${filter}`);
