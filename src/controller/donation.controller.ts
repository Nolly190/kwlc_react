      ...
      for (let k = 0; k < data.length; k++) {
        ...
        if (singleResponse.status) {
          ...
          const _donationData = new DonateItemDTO({
            ...
            image: singleResponse?.data?.donationImages?.filter(
              (x) => x.isMainImage
            )[0]?.imageUrl,
  ...
  try {
    ...
    setItem(
      new DonateItemDTO({
        ...
        id: i.id,
        image:
          i.donationImages?.length > 0
            ? i.donationImages?.filter((x) => x.isMainImage)[0].imageUrl
            : "",
        images:
          i.donationImages?.length > 0
            ? i.donationImages?.map((x) => x.imageUrl)
            : [],
        raised: 0,
        target: 0,
        title: i.title,
